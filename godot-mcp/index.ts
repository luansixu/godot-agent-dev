...
class GodotServer {
  ...
  private previousScreenshot: { base64: string; width: number; height: number; timestamp: number } | null = null;
  private watchedNodes: Map<string, { projectPath: string; nodePath: string; properties: string[]; lastValues: Map<string, any> }> = new Map();
   ...
   */
  private async handleCaptureGameScreenshot(args: any) {
    args = this.normalizeParameters(args);
...
`.trim();
    ...
    try {
      ...
      if (result.startsWith('ERROR:')) {
        ...
        if (msg === 'WINDOW_NOT_FOUND') {
          return this.createErrorResponse(
            `No Godot game window found with title containing "${windowTitle}"`,
            [
   ...
   */
  private async handleCaptureEditorScreenshot(args: any) {
    args = this.normalizeParameters(args);
...
`.trim();
   ...
   */
  private async handleCaptureGameScreenshotDiff(args: any) {
    args = this.normalizeParameters(args);
    const windowTitle = args.windowTitle || '昼与夜';
...
`.trim();
    ...
    try {
      ...
      return this.createErrorResponse(
        `Failed to capture screenshot diff: ${error?.message || 'Unknown error'}`,
        ['Ensure Python, win32gui, and PIL are installed']
      );
    }
  }
  ...
  private async handleInputSequence(args: any) {
    ...
    const actionKeyMap: Record<string, number> = {
    ...
    const inputsPyList = args.inputs.map((input: any) => {
...
`.trim();
    ...
    try {
      ...
      if (stdout.trim().startsWith('ERROR:')) {
        ...
        if (msg === 'WINDOW_NOT_FOUND') {
          return this.createErrorResponse(
            `No window found with title containing "${windowTitle}"`,
            [
      ...
      return this.createErrorResponse(
        `Failed to inject input: ${error?.message || 'Unknown error'}`,
        [
   ...
   */
  private async handleGetRuntimeState(args: any) {
    args = this.normalizeParameters(args);
    ...
    const propsCondition = properties.length > 0
      ? `var props = [${properties.map((p: string) => `"${p}"`).join(', ')}]; var result = {}\nfor p in props:\n    if n.has(p): result[p] = n.get(p)\nreturn JSON.stringify(result)`
...
`;
    ...
    try {
      const { stdout, stderr } = await execFileAsync(
        this.godotPath!,
        ['--headless', '--path', args.projectPath, '--script', tmpScript],
        { timeout: 10000 }
      );
      ...
      if (jsonStart === -1) {
        return this.createErrorResponse(
          `Unexpected Godot output: ${output.substring(0, 200)}`,
          []
        );
      }
      ...
      return this.createErrorResponse(
        `Failed to query runtime state: ${error?.message || 'Unknown error'}`,
        ['Ensure the game is running and the node path is correct']
      );
    } finally {
   ...
   */
  private async handleWatchNode(args: any) {
    args = this.normalizeParameters(args);
    ...
    if (currentState.content?.[0]?.type === 'text' && (currentState.content[0] as any).text.startsWith('ERROR')) {
      return currentState;
    }

    // Parse current values from the response
    const text = (currentState as any).content?.[0]?.text || '';
    const jsonMatch = text.match(/\{[\s\S]*\}/);
    let currentValues: Map<string, any> = new Map();
    if (jsonMatch) {
      try {
   ...
   */
  private async handleGetWatchResults(args: any) {
    const watchId = args?.watchId;
    const results: Array<{ watchId: string; nodePath: string; changed: boolean; changes: Array<{ property: string; oldValue: any; newValue: any }> }> = [];
    ...
    const watchEntries = watchId
      ? [[watchId, this.watchedNodes.get(watchId)!] as [string, any]]
    ...
    const summary = results.map(r => {
      if (!r.changed) return `[${r.watchId}] ${r.nodePath}: no changes`;
